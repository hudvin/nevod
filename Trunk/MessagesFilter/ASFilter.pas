   ...
   begin       // делать выбоку по типу фильтра
    ...
    while (not Res) and (not Eof) do
     begin
      ...
      FExp.ShieldingExp:=FieldByName('FValue').AsString; // сыра€ маска получателс€
      FExp.Subject:=Sender;
...
function TSignalFilter.AnalyzeMessage(Mess:TFMessage): Boolean;
 function GetRowText(MessageText:String):String;  // удал€ет теги из текста и лишние пробелы
 var buff:String;
 begin
  buff:=MessageText;
  with FExp  do
...
if Mess.BodyType=btHtml then
  RowText:=GetRowText(Mess.MessageText) // получение текста сообщени€ без тегов и лишних пробелов
 else RowText:=Mess.MessageText;
 FProc.Active:=False;
 FType:=GetEnumName(TypeInfo(TFilterType), Ord(FilterType));
 case FSignalLocation of  // по каким пол€м сообщени€ должен производитс€ поиск
  slAnywhere:
   begin
...
begin
  if Mess.MessageText<>'' then
   ...
   Flag:=True;
   FExp.RegEx:='<\w*img';
  // Exp.Subject:='<img sr=> <img>  <img srcgf>   fvsdfs'; // убрать нафиг - только дл€ проверки
   FExp.Subject:=Mess.MessageText;
   if FExp.Match then
    repeat
     inc(i);
...
begin
  inherited Create(Exp,Proc,FilterType);
 { with Proc do   // выборка всех расширений дл€ фильтра данного типа
   begin
    Active:=False;
 ...
 while (not Flag) AND  (i<Mess.MessageParts.Count) do
  begin
    if Mess.MessageParts.Items[i] is TIdAttachment then
      with FProc do   // поиск данного расширени€ в таблице
       begin
...
begin
  {
  идентичный код дл€  TAllowFilterGroup и TDenyFilterGroup
  дл€ контекстного фильтра - перезаписать метод
  }
 Flag:=False;
 i:=0;
