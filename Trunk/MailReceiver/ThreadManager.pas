    ...
    begin
     if TBaseReceiver(PostReceivers[i]).Terminated then
      with TBaseReceiver(PostReceivers[i]) do
        ...
        // статус обновл€ест€ через AccountManager
         if SuccessFul=False then
          begin
           Logger.Add(LogMessage,AccountId,ltPostReceiver);
          
           SendHintMessage(AccountParams.AccountName,
               _(' ќшибка при получении почты дл€  ')+ '"'+AccountParams.AccountName +'"',
               LogMessage,
               bitError,
               MessagesCount);
          end
            ...
            begin
             ...
             Mess.Caption:=_(' ѕолучена нова€ почта  дл€  ')+ '"'+AccountParams.AccountName +'"';
             Mess.LogMessage:=_(' «агружено ')+  ' '+IntToStr(MessagesCount)+ _(' новых писем');
             Mess.MessagesCount:=MessagesCount;
             Mess.BallonType:=bitInfo;
             SendHintMessage(
                AccountParams.AccountName,
                _(' ѕолучена нова€ почта  дл€  ')+ '"'+AccountParams.AccountName +'"',
                _(' «агружено ')+  ' '+IntToStr(MessagesCount)+ _(' новых писем'),
                bitInfo,
                MessagesCount
             );
            end;
